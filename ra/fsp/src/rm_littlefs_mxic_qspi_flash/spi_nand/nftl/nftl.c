...
#include <stdio.h>
#include <string.h>
#include "../spi_nand_app.h"

#include "../../spi_nand/nftl/inc/nftl_conf.h"
                                 ...
                                 void *spare_buf, uint32_t spare_buf_len)
{
    ...
    ret = BSP_SPI_Read(&Mxic, (u32)(*off_set), data_buf_len, (u8 *)data_buf);
    if (ret != 0) {
    NFTL_ERROR("[%s] read data error, off_set: %lx, data_buf_len: %lx", __func__, *off_set, data_buf);
    return ret;
    }

    ret = BSP_SPI_ReadOob(&Mxic, (u32)(*off_set), spare_buf_len, (u8 *)spare_buf, TAG_OFFSET);
    if (ret != 0) {
    NFTL_ERROR("[%s] read spare error, off_set: %lx, spare_buf_len: %lx", __func__, (*off_set + NFTL_PAGE_SIZE + TAG_OFFSET), spare_buf_len);
    return ret;
    }
                                 ...
                                 void *spare_buf, uint32_t spare_buf_len)
{
    ...
    ret = BSP_SPI_Write(&Mxic, *off_set, data_buf_len, (u8 *)data_buf);
    if (ret != 0) {
    NFTL_ERROR("[%s] program data error, off_set: %lx, data_buf_len: %lx", __func__, *off_set, data_buf_len);
    return ret;
    }

    ret = BSP_SPI_WriteOob(&Mxic, (u32)(*off_set), spare_buf_len, (u8 *)spare_buf,  TAG_OFFSET);
    if (ret != 0) {
    NFTL_ERROR("[%s] program spare error, off_set: %lx, spare_buf_len: %lx", __func__, (*off_set + NFTL_PAGE_SIZE + TAG_OFFSET), spare_buf_len);
    return ret;
    }
...
{
    int ret;
    uint32_t addr = (off_set / NFTL_BLOCK_OFFSET * NFTL_BLOCK_OFFSET);
    ret = BSP_SPI_Block_Is_Bad(&Mxic, addr);
    if(ret == MXST_BLOCK_IS_BAD)
    {
        return true;
    }
    else
...
int hal_flash_markbad(hal_partition_t in_partition, uint32_t off_set)
{
    int ret;
    uint32_t addr = (off_set / NFTL_BLOCK_OFFSET * NFTL_BLOCK_OFFSET);
    ret = BSP_SPI_Block_Mark_Bad(&Mxic, addr);
    return ret;
}
...
static void nftl_blk_release(nftl_block_mgr_t *blk_mgr ,uint32_t phy_blk)
{
    ...
    if (0 == count_valid_page(blk_mgr, phy_blk)) {
        ret = BSP_SPI_Erase_Block(&Mxic, phy_blk * NFTL_BLOCK_OFFSET, NFTL_BLOCK_SIZE/0x20000);
...
static nftl_err_t nftl_immigrate_block(nftl_block_mgr_t *blk_mgr, uint32_t old_blk)
{
    ...
    ret = BSP_SPI_Erase_Block(&Mxic, old_blk * NFTL_BLOCK_OFFSET, NFTL_BLOCK_SIZE/0x20000);
...
static void nftl_update_tagpage(nftl_block_mgr_t *blk_mgr, uint32_t blk)
{
    ...
    do {
        ...
        /* TODO: deal with endian */
        ret = BSP_SPI_Write(&Mxic, w_off, sizeof(nftl_block_tag) * NFTL_TAG_PAGE_IN_BLOCK, (u8 *)blk_tags);
...
static uint32_t nftl_alloc_new_block(nftl_block_mgr_t *blk_mgr)
{
    ...
    do {
        ...
        ret = BSP_SPI_Erase_Block(&Mxic, offset, NFTL_BLOCK_SIZE/0x20000);
...
static nftl_err_t nftl_init_partition(nftl_block_mgr_t *blk_mgr)
{
    ...
    for (i = 0; i < blk_mgr->phy_block_nums; i++) {
        ...
        /* TODO: deal with endian */
        ret = BSP_SPI_Read(&Mxic, r_off, NFTL_PAGE_SIZE, blk_mgr->page_buff);
        r_off += NFTL_PAGE_OFFSET;
                    ...
                    void *outbuf, nftl_size_t len)
{
    ...
    while (len > 0) {
        ...
        /* TODO: deal with endian */
        ret = BSP_SPI_Read(&Mxic, r_off, r_len, p_buf);
        r_off += r_len;
...
int nftl_flash_eraseall(nftl_partition_t p)
{
    ...
    for (uint32_t blk = 0; blk < blk_mgr->phy_block_nums; blk++) {
        ret = BSP_SPI_Erase_Block(&Mxic, blk * NFTL_BLOCK_OFFSET, NFTL_BLOCK_SIZE/0x20000);
        if (ret != 0) {
