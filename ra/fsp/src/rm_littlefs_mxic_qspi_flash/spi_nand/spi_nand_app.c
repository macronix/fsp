 ...
 *                offset:    oob structure parameter
 * Return Value:  MXST_SUCCESS
 *                MXST_FAILURE
 * Description:   This function readS data and/or out-of-band.
 */
int BSP_SPI_ReadOob(MxChip *Mxic, u32 Addr, u32 ByteCount, u8 *Buf, u32 offset)
{
    int Status;
 ...
 */
int BSP_SPI_WriteOob(MxChip *Mxic, u32 Addr, u32 ByteCount, u8 *Buf, u32 offset)
{
    int Status;
    struct Mtd_Oob_Ops Ops;
...
int BSP_SPI_Nand_Init(MxChip *Mxic, const spi_flash_instance_t * controller)
{
    ...
    Status = Mx_Nand_EccInit(Mxic, NAND_ECC_NONE);
    if (Status != MXST_SUCCESS)
        return Status;
